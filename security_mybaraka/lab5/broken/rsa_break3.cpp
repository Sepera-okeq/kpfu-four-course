...
BigInt pollard_pm1(const BigInt& N) {
    vector<string> bases = {"2", "3", "5", "7"};  // Уменьшаем количество баз для маленьких чисел
    
    cout << "Starting factorization..." << endl;
    
    for (const string& base_str : bases) {
        BigInt base(base_str);
        cout << "Trying base " << base_str << endl;
        
        // Начинаем с маленького показателя степени
        BigInt power = base % N;
        
        for (int i = 2; i <= 10; i++) {  // Небольшое количество итераций для маленьких чисел
            power = mod_pow(power, BigInt(i), N);  // Используем предыдущий результат
            BigInt d = gcd(power - BigInt("1"), N);
            
            if (d != BigInt("1") && d != N) {
                cout << "Found factor with base " << base_str 
                     << " after " << i-1 << " iterations" << endl;
                return d;
            }
            
            cout << "Base " << base_str << ", iteration " << i << endl;
        }
    }
...
int main() {
    ...
    // Поиск множителей с помощью оптимизированного алгоритма Полларда p-1
    BigInt p = pollard_pm1(N);
