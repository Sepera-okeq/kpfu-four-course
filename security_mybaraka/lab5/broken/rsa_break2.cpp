...
void pollard_thread(const BigInt& N, const BigInt& start_base, atomic<bool>& found, 
                    BigInt& result, mutex& mtx) {
    BigInt base = start_base;
    int itr = 2;
    int local_iterations = 0;
    
    while (!found) {
        base = mod_pow(base, BigInt(itr), N);
        BigInt d = gcd(base - BigInt("1"), N);
        ...
        if (d != BigInt("1") && d != N) {
            ...
            if (!found) {
                result = d;
                found = true;
                cout << "Thread with base " << start_base 
                     << " found factor after " << local_iterations << " iterations!" << endl;
            }
            break;
        }
        ...
        if (local_iterations % 100 == 0) {
            lock_guard<mutex> lock(mtx);
            cout << "Base " << start_base << ": iteration " << local_iterations << endl;
        }
...
BigInt parallel_pollard_pm1(const BigInt& N) {
    vector<BigInt> bases = {
        BigInt("2"), BigInt("3"), BigInt("5"), BigInt("7"),
        BigInt("11"), BigInt("13"), BigInt("17"), BigInt("19"),
        BigInt("23"), BigInt("29"), BigInt("31"), BigInt("37")
    };
    ...
    for (const BigInt& base : bases) {
        threads.emplace_back(pollard_thread, ref(N), ref(base), ref(found), ref(result), ref(mtx));
    }
...
int main() {
    ...
    // Поиск множителей с помощью параллельного алгоритма Полларда p-1
    BigInt p = parallel_pollard_pm1(N);
