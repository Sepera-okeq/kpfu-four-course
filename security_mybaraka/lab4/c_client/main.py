...
Модуль реализует клиентскую часть приложения, включая:
- Сетевое взаимодействие с сервером
- Регистрацию и аутентификацию пользователей
- Шифрование и обмен сообщениями
- Цифровую подпись файлов

Протокол взаимодействия:
1. Регистрация:
   - Отправка: REGISTER|login|password
   - Ответ: SUCCESS/EXISTS

2. Аутентификация:
   - Отправка: AUTH|login
   - Получение: hash(SW)
   - Отправка: hash(hash(SW) + hash(password))
   - Ответ: SUCCESS/WRONG_PASSWORD
   - При успехе: обмен ключами Диффи-Хеллмана

3. Обмен сообщениями:
   - Шифрование RC4 с сеансовым ключом
   - Поддержка текстовых сообщений
   - Цифровая подпись файлов через RSA

Основные классы:
- MessageThread: Поток для асинхронного приема сообщений
- ClientObject: Основной класс клиента для работы с сетью и криптографией
