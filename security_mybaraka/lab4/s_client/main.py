...
Модуль реализует серверную часть приложения, включая:
- Обработку подключений клиентов
- Управление пользователями через БД
- Аутентификацию и генерацию SW
- Обмен ключами и шифрование

Протокол взаимодействия:
1. Регистрация:
   - Прием: REGISTER|login|password
   - Проверка существования пользователя
   - Хеширование пароля и сохранение в БД
   - Ответ: SUCCESS/EXISTS

2. Аутентификация:
   - Прием: AUTH|login
   - Генерация SW и сохранение в БД
   - Отправка: hash(SW)
   - Проверка: hash(hash(SW) + hash(password))
   - Ответ: SUCCESS/WRONG_PASSWORD
   - При успехе: инициация обмена ключами DH

3. Обмен сообщениями:
   - Шифрование RC4 с сеансовым ключом
   - Проверка цифровых подписей файлов
   - Поддержка текстовых сообщений

Основные классы:
- ServerThread: Поток для асинхронной обработки подключений
- ServerObject: Основной класс сервера для работы с сетью и БД
- ChatWindow: GUI окно чата с поддержкой шифрования
